"""Normalization pipeline for patronymics.

Rules (same baseline as names/surnames):
- lowercase, `ё->е`, `й->и`
- collapse 3+ identical chars to 1
- transliterate Latin homographs to Cyrillic; other non-Russian -> '*'
- hyphen -> space; process parts independently
...
def normalize_patronymic(value: object) -> Optional[str]:
...
def normalize_series(
    ...
    if processes is None or processes < 1:
        processes = 1
    if processes == 1:
        return [normalize_patronymic(v) for v in tqdm(data, desc="Normalizing patronymic", unit="patr")]
    chunk_size = max(2000, len(data) // (processes * 8) or 1)
    tqdm.write(f"Using {processes} processes, chunksize={chunk_size}")
    with mp.Pool(processes=processes) as pool:
        iterator = pool.imap(normalize_patronymic, data, chunksize=chunk_size)
        return list(tqdm(iterator, total=len(data), desc="Normalizing patronymic", unit="patr"))


def main():
    parser = argparse.ArgumentParser(description="Normalize patronymic column.")
    parser.add_argument("--input", required=True, help="Input file (parquet/csv)")
    parser.add_argument("--column", default="fathername", help="Patronymic column")
    parser.add_argument(
        "--output",
        default="clean_data/clean_patronymic/normalized.parquet",
